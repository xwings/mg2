...
original data. `parseSJN` deliberately keeps non-F0 records as raw
bytes for a future tile-rewrite pass (M3, see
[npc-state.md](npc-state.md)).
...
- **M3**: `parseSJN` non-F0 records kept as `rawTiles` raw bytes,
  awaiting the tile-rewrite pass in [npc-state.md](npc-state.md).
